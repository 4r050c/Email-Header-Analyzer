...
(function (global) {
  ...
  function runAnalysis(rawText) {
    ...
    try {
      ...
      const subject = EHA.parser.decodeRFC2047(EHA.parser.firstValue(headerMap, "subject"));
      ...
      const mailMeta = EHA.heuristics.extractMailMeta(headerMap);
      ...
      const analysis = {
        ...
        authAnalysis, espMatches, flags, risk, mailMeta,
      };
      ...
      // Move focus into the report so screen readers land on the freshly
      // rendered summary instead of staying silently on the Analyze button.
      document.getElementById("summary-banner").focus({ preventScroll: true });
      switchTab("overview");
  ...
  // Key includes the target's index because safeDomId is not injective --
  // "a.b.com" and "a-b.com" would otherwise collapse to the same DOM id and
  // one result card would silently overwrite the other. Both this and
  // render.js derive keys from the same collectBlacklistTargets array, so
  // index-based ids stay in sync.
  function blacklistKey(target, index) {
    return `blacklist-${index}-${EHA.parser.safeDomId(target.value)}`;
  }
  ...
  async function kickOffEnrichment(analysis) {
    ...
    if (!settings.liveLookups || !settings.apiKeys.length) {
      ...
      blacklistTargets.forEach((t, i) => EHA.render.renderMxResult(blacklistKey(t, i), { status: "disabled", message, argument: t.value }));
      return;
    }
    ...
    const config = {
      ...
      // Reconcile against a FRESH read of settings rather than persisting
      // this closure's snapshot wholesale -- lookups are async, and the user
      // may have added/removed/edited keys in Settings while one was in
      // flight; writing the stale array back would revive deleted keys or
      // revert edits. Only the mutated key's exhaustion state is merged, by id.
      onKeyExhausted: (keyEntry) => {
        const fresh = loadSettings();
        const target = fresh.apiKeys.find((k) => k.id === keyEntry.id);
        if (target) {
          target.exhaustedUntil = keyEntry.exhaustedUntil || {};
          saveSettings({ apiKeys: fresh.apiKeys });
        }
    ...
    blacklistTargets.forEach(async (t, i) => {
      const key = blacklistKey(t, i);
  ...
  function initTheme() {
    const settings = loadSettings();
    // Must live on <html>: the CSS light palette targets :root[data-theme="light"].
