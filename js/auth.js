...
(function (global) {
  ...
  // forwarders) that would otherwise break SPF/DKIM. Each hop adds a numbered
  // instance (i=N) of three headers: ARC-Authentication-Results (the auth
  // results that hop saw), ARC-Message-Signature, and ARC-Seal (whose cv=
  // records the chain validation state at that hop).
  function parseArcTags(value) {
    const tags = {};
    value.split(";").forEach((seg) => {
      const m = seg.trim().match(/^([a-zA-Z0-9]+)\s*=\s*([\s\S]*)$/);
      if (m) tags[m[1].toLowerCase()] = m[2].trim().replace(/\s+/g, (m[1].toLowerCase() === "b" || m[1].toLowerCase() === "bh") ? "" : " ");
    });
    return tags;
  }

  function analyzeArc(headerMap) {
    const seals = EHA.parser.allValues(headerMap, "arc-seal").map(parseArcTags);
    const messageSigs = EHA.parser.allValues(headerMap, "arc-message-signature").map(parseArcTags);
    const authResults = EHA.parser.allValues(headerMap, "arc-authentication-results");
    if (!seals.length && !messageSigs.length && !authResults.length) {
      return { present: false, instances: [], chainValidation: null };
    }
    const byInstance = new Map();
    function slot(i) {
      const key = String(i || "?");
      if (!byInstance.has(key)) byInstance.set(key, { instance: i, seal: null, messageSig: null, authResults: null });
      return byInstance.get(key);
    }
    seals.forEach((s) => { slot(s.i).seal = s; });
    messageSigs.forEach((s) => { slot(s.i).messageSig = s; });
    authResults.forEach((raw) => {
      const iMatch = raw.match(/\bi\s*=\s*(\d+)/);
      const i = iMatch ? iMatch[1] : "?";
      const stripped = raw.replace(/^\s*i\s*=\s*\d+\s*;?\s*/, "");
      slot(i).authResults = parseAuthenticationResultsValue(stripped);
    });
    const instances = Array.from(byInstance.values()).sort((a, b) => Number(a.instance || 0) - Number(b.instance || 0));
    // The highest-instance ARC-Seal's cv= is the overall chain state:
    // none (no prior chain), pass (valid), or fail (broken).
    const topSeal = instances.filter((x) => x.seal).map((x) => x.seal).slice(-1)[0];
    const chainValidation = topSeal && topSeal.cv ? topSeal.cv.toLowerCase() : null;
    return { present: true, instances, chainValidation };
  }
  ...
  function analyze(headerMap, fromAddress) {
    ...
    const arc = analyzeArc(headerMap);

    return {
      arHeaders, receivedSpfHeaders, dkimSigHeaders, arc,
