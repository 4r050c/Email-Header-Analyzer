...
(function (global) {
  ...
  function riskCell(risk, flagCount) {
    ...
    svg.setAttribute("aria-hidden", "true"); // decorative; score + band are real text beside it
  ...
  function renderOverview(a) {
    ...
    root.replaceChildren(el("div", { class: "grid-2" }, [details, delivery]), mailMetaCard(a), flagsPreview);
  }

  function mailMetaCard(a) {
    const meta = a.mailMeta;
    if (!meta) return null;
    const rows = [];
    if (meta.list) {
      rows.push(kvRow("Mailing List", meta.list.id || "(unnamed list)"));
      if (meta.list.unsubscribe) {
        const urlMatch = meta.list.unsubscribe.match(/<(https?:[^>]+)>/);
        rows.push(el("tr", {}, [
          el("th", {}, ["Unsubscribe"]),
          el("td", { class: "copyable" }, [
            urlMatch ? el("a", { href: urlMatch[1], target: "_blank", rel: "noopener noreferrer" }, [meta.list.oneClick ? "One-Click unsubscribe link ↗" : "Unsubscribe link ↗"]) : el("span", {}, [meta.list.unsubscribe]),
            meta.list.oneClick ? el("span", { class: "tag tag-spaced" }, ["RFC 8058 One-Click"]) : null,
          ].filter(Boolean)),
        ]));
      }
    }
    meta.spam.forEach((s) => rows.push(kvRow(s.system, s.score ? `${s.value}` : s.value)));
    if (meta.precedence) rows.push(kvRow("Precedence", meta.precedence));
    if (meta.autoSubmitted) rows.push(kvRow("Auto-Submitted", meta.autoSubmitted));
    if (meta.priority) rows.push(kvRow("Priority", meta.priority));
    if (!rows.length) return null;
    return el("div", { class: "card" }, [
      el("div", { class: "card-title" }, [el("h3", {}, ["List, Bulk & Spam-Filter Signals"])]),
      el("p", { class: "card-desc" }, ["Mailing-list identity and any spam-filter verdicts already stamped onto the message by intermediate servers."]),
      el("table", { class: "kv-table" }, rows),
    ]);
  }
  ...
  function hopNode(hop) {
    ...
    if (hop.tls === true) {
      routeChildren.push(el("span", { class: "tls-badge tls-on", title: hop.tlsVersion ? `Encrypted (${hop.tlsVersion}${hop.tlsCipher ? ", " + hop.tlsCipher : ""})` : "Encrypted in transit (TLS)" }, ["🔒 TLS"]));
    } else if (hop.tls === false) {
      routeChildren.push(el("span", { class: "tls-badge tls-off", title: "No TLS indicated on this hop" }, ["⚠ no TLS"]));
    }
    const route = el("div", { class: "hop-route" }, routeChildren);
    const metaItems = [];
    if (hop.fromIP) metaItems.push(metaItem("IP", hop.fromIP));
    if (hop.protocol) metaItems.push(metaItem("Protocol", hop.protocol));
    if (hop.tlsVersion) metaItems.push(metaItem("TLS", hop.tlsVersion + (hop.tlsCipher ? " / " + hop.tlsCipher : "")));
  ...
  function mxPlaceholder(command) {
    return el("div", { class: "mx-subcard", id: `mx-card-${command}` }, [
      ...
      el("div", { id: `mx-body-${command}`, "aria-live": "polite" }, [el("div", { class: "empty-state" }, ["Not run. Enable live lookups and an API key in Settings."])]),
    ]);
  }
  ...
  function blacklistTargetSubcard(target, index) {
    // Index-prefixed ids: safeDomId is not injective ("a.b.com" vs "a-b.com"),
    // and app.js builds its render keys the same way from the same array.
    const safeId = `${index}-${EHA.parser.safeDomId(target.value)}`;
    return el("div", { class: "mx-subcard", id: `mx-card-blacklist-${safeId}` }, [
      ...
      el("div", { id: `mx-body-blacklist-${safeId}`, "aria-live": "polite" }, [el("div", { class: "empty-state" }, ["Not run. Enable live lookups and an API key in Settings."])]),
    ]);
  }
  ...
  function liveBlacklistCard(a) {
    ...
    const body = targets.length
      ? el("div", { class: "grid-2" }, targets.map((t, i) => blacklistTargetSubcard(t, i)))
  ...
  function arcCard(arc) {
    ...
    const cvTone = { pass: "good", none: "neutral", fail: "critical" }[arc.chainValidation] || "neutral";
    const instanceRows = arc.instances.map((inst) => {
      const bits = [];
      if (inst.seal) bits.push(`seal cv=${inst.seal.cv || "?"} d=${inst.seal.d || "?"}`);
      if (inst.messageSig) bits.push(`sig d=${inst.messageSig.d || "?"} s=${inst.messageSig.s || "?"}`);
      if (inst.authResults) {
        const methods = inst.authResults.results.map((r) => `${r.method}=${r.result}`).join(" ");
        if (methods) bits.push(`orig auth: ${methods}`);
      }
      return el("div", { class: "auth-mech" }, [
        el("div", { class: "auth-mech-badge" }, [el("span", { class: "tag" }, [`i=${inst.instance || "?"}`])]),
        el("div", { class: "auth-mech-body" }, bits.map((b) => el("div", { class: "auth-mech-detail" }, [b]))),
      ]);
    });
    return el("div", { class: "card" }, [
      el("div", { class: "card-title" }, [el("h3", {}, ["ARC (Authenticated Received Chain)"]), pill(`cv=${arc.chainValidation || "n/a"}`, cvTone)]),
      el("p", { class: "card-desc" }, ["Added by forwarders / mailing lists to preserve the original authentication results across hops that would otherwise break SPF and DKIM. The chain-validation state (cv) on the newest seal tells you whether that preserved history can be trusted."]),
      ...instanceRows,
    ]);
  }
  ...
  function renderAuth(a) {
    ...
    root.replaceChildren(...[spfCard, dkimCard, dmarcCard, arcCard(auth.arc), dkimSignatureCard(auth.dkimSigHeaders), liveDnsCard(a), liveBlacklistCard(a)].filter(Boolean));
  }
  ...
  function downloadAnalysisJSON(a) {
    const exportObj = {
      ...
      hops: a.hops.map((h) => ({
        ...
        protocol: h.protocol, tls: h.tls, tlsVersion: h.tlsVersion, tlsCipher: h.tlsCipher,
        delaySeconds: h.delaySeconds, timestamp: h.timestamp ? h.timestamp.toISOString() : null,
      })),
      totalSeconds: a.totalSeconds,
      authentication: { spf: a.authAnalysis.primarySpf, dkim: a.authAnalysis.primaryDkim, dmarc: a.authAnalysis.primaryDmarc },
      arc: a.authAnalysis.arc && a.authAnalysis.arc.present ? { chainValidation: a.authAnalysis.arc.chainValidation, instanceCount: a.authAnalysis.arc.instances.length } : null,
      mailMeta: a.mailMeta,
  ...
  function checkRow(tone, item, fallbackLabel) {
    ...
    // Scheme allowlist: the Url comes from an external API response (or a
    // user-configured proxy), so never let a javascript:/data: value become
    // a clickable href.
